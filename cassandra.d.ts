...
declare module "cassandra" {
  ...
  namespace types {
    ...
    interface TupleStatic<R, S, T> {
      new (...arguments: [R, S, T]): Tuple<R,S,T>;

      fromArray(elements: [R, S, T]): Tuple<R, S, T>;
    }

    interface Tuple<R, S, T> {
      elements: [R, S, T];
      length: number;
      
      get(index: number): R|S|T;
      toString(): string;
      toJSON(): string;
      values(): [R, S, T];
    }
